...
\begin{enumerate}
 ...
 \item models for testing the frequency server (extracted from \cite{CesariniThompson})
\end{enumerate}
...
We now evaluate the results in more detail, examining some of the trade-offs and 
possible alternatives (and future improvements) in applying these techniques in practice.
...
since the difference was small and located in one single place. Parametrisation 
is done automatically in exchange for some automatically generated interface 
boilerplate.

The manual approach for frequency server allows to keep the 
interface of both modules in the common side, but parametrisation of the 
...
Frequency server example was simple, the automatic approach 
already generated a good result by itself and we could see that the only 
...
In the automatic approach all the functions that existed, which in this case
had common body, were replaced in both instances with simple function calls 
that simply redirect the original interface to the common part, in order to 
keep the original behaviour. This is good because we would not have to modify 
anything, else, it works the same as it did, but we have a small amount of 
...
the code that changes in one of the instances, (preferably the most complex 
...
the code corresponding to the other version. If we selected the most complex 
...
must yet generalise the calls to the \emph{behaviour instance} callbacks. In 
...
understand for humans, specially because generated callbacks do not have 
...
Parametrising the models for \texttt{ets} and \texttt{dest} was more 
